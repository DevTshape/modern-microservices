...
import (
	...
	"github.com/DevTshape/modern-microservices/auth/redis"
...
func DeleteTokens(authD *AccessDetails) error {
	...
	//delete access token
	deletedAt, err := redis.Client.Del(authD.AccessUuid).Result()
	if err != nil {
		return err
	}
	//delete refresh token
	deletedRt, err := redis.Client.Del(refreshUuid).Result()
	if err != nil {
		return err
	}
...
func Refresh(c *gin.Context) {
	...
	if ok && token.Valid {
		...
		saveErr := CreateAuth(uint(userId), ts)
		if saveErr != nil {
